               ...
               .then(() => {
                  if (chrome.runtime.lastError) {
                     chrome.tabs.query(
                        ...
                        function (tabs) {
                           var url = tabs[0].url;
                           trackEvent("scan_job_desc", { action: "errorGetPageSource", page_url: url });
...
//    });
// }
function _getSourceFromHostname(hostname) {
...
function _fireNoKeywordsEvent(url, resume_text, job_text) {
   try {
      var pageUrl = (typeof url === "string") ? url : url.toString();
      var hostname = "";
      try { hostname = new URL(pageUrl).hostname; } catch(e) {}
      import(chrome.runtime.getURL("/js/analytics.js")).then(function(analytics) {
         analytics.trackEvent("scan_error_no_keywords", {
            page_url: pageUrl,
            source: _getSourceFromHostname(hostname),
            resume_length: String(resume_text.length),
            job_text_length: String(job_text.length),
         });
      });
   } catch(e) {}
}
...
function get_match_result(url, resume_text, job_text) {
   ...
   if (domain == "swe") {
      ...
      var result = match_result_via_weighted_algorithm(
         resume_essentials,
         resume_nice_to_haves,
         job_essentials,
         job_nice_to_haves
      );
      if (job_essentials.length === 0 && job_nice_to_haves.length === 0) {
         _fireNoKeywordsEvent(url, resume_text, job_text);
      }
      return result;
   }
