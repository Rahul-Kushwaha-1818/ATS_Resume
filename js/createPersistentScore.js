...
function _getSource() {
   ...
   if (url.hostname.includes("myworkdayjobs.com")) return "workday";
   if (url.hostname.includes("joinhandshake.com")) return "handshake";
   return "other";
}
...
async function save_job_and_maybe_create_persistent_score() {
         ...
         function (data) {
            ...
            if (reviewLink) {
               reviewLink.addEventListener("click", function () {
                  _getAnalytics().then(function(analytics) {
                     analytics.trackEvent("resume_review_click", { source_page: "persistent_score" });
                  });
               });
            }

            _getAnalytics().then(function(analytics) {
               analytics.trackEvent("persistent_score_displayed", {
   ...
   } else if (jobText === "" && target_elm != null) {
      _getAnalytics().then(function(analytics) {
         analytics.trackEvent("scan_error_no_job_text", {
            page_url: window.location.href,
            source: _getSource(),
         });
      });
